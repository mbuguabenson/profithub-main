    ...
    const getLogMessage = () => {
        switch (logType) {
            ...
            case LogTypes.PURCHASE: {
                const { transaction_id, contract_type } = extra;
                return (
                    <Localize
                        i18n_default_text='<0>Bought</0>: {{contract_type}} contract purchased (ID: {{transaction_id}})'
                        values={{ transaction_id, contract_type: contract_type || 'UNKNOWN' }}
