...
export default class TradingTimes {
    ...
    async initialise() {
        ...
        if (!Object.keys(this.trading_times).length) {
            ...
            // Do NOT await — periodicUpdate schedules itself via setTimeout
            // and blocking here doubles the init time unnecessarily.
            periodicUpdate();
        }
    ...
    async updateTradingTimes() {
        ...
        try {
            ...
            const timeout = new Promise((_, reject) =>
                setTimeout(() => reject(new Error('trading_times request timeout')), 10000)
            );
            const request =
