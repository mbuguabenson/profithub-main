...
function connect() {
    const endpoint = ENDPOINTS[ENDPOINT_INDEX % ENDPOINTS.length];
    ws = new WebSocket(endpoint);

    ws.onopen = () => {
        ws.send(JSON.stringify({ active_symbols: 'brief' }));
        const symToSub = window.localStorage.getItem(CIRCLES_SYMBOL_KEY) || currentSymbol || '1HZ10V';
        currentSymbol = null;
        subscribe(symToSub);
    };
    ...
    ws.onmessage = msg => {
        ...
        if (data.tick) {
            ...
            if (symbol === currentSymbol) {
                ...
                if (priceEl) priceEl.innerHTML = formatPriceWithHighlight(quote, usedDecimals);
        ...
        if (data.error) {
            ...
            if (data.error.code === 'InvalidSymbol' && currentSymbol) {
                failedSymbols.add(currentSymbol);
                const candidates = ['1HZ10V', '1HZ100V', 'R_100', 'R_10'];
                const fallback = candidates.find(s => !failedSymbols.has(s));
                if (fallback && fallback !== currentSymbol) {
                    console.warn(`Falling back to symbol ${fallback} due to InvalidSymbol on ${currentSymbol}`);
                    subscribe(fallback);
                } else {
                    console.warn(`No valid fallback symbol available for ${currentSymbol}`);
                }
            }
        }
    };
