...
export default class AutoTraderStore {
    ...
    executeCustomTrade = async (type: string, prediction: number) => {
        ...
        try {
            ...
            // Monitor contract result via WebSocket subscription with cleanup and safety timeout
            let isFinished = false;
            let subObj: any = null;

            const cleanupSub = () => {
                if (subObj && typeof subObj.unsubscribe === 'function') {
                    try { subObj.unsubscribe(); } catch (e) {}
                }
                if (contract_id) {
            ...
            const subPromise = api_base.api.subscribe(
