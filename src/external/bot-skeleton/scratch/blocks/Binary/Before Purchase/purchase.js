...
window.Blockly.Blocks.purchase = {
    ...
    definition() {
        return {
            ...
            message2: localize('No. of Trades: {{ count }}', { count: '%1' }),
            args2: [
                {
                    type: 'field_number',
                    name: 'BULK_TRADES_COUNT',
                    value: 1,
                    min: 1,
                    max: 10,
                    precision: 1,
...
window.Blockly.JavaScript.javascriptGenerator.forBlock.purchase = block => {
    ...
    const allowBulk = block.getFieldValue('ALLOW_BULK') || 'no';
    const bulkCount = block.getFieldValue('BULK_TRADES_COUNT') || 1;

    const code = `
        Bot.purchase_block_allow_bulk = '${allowBulk}';
        Bot.purchase_block_bulk_count = ${bulkCount};
