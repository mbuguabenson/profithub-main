...
const Matches = observer(() => {
    ...
    useEffect(() => {
        ...
        const init = async () => {
            try {
                const apiInstance = await generateDerivApiInstance();
                apiRef.current = apiInstance;
                setApi(apiInstance);

                // Load symbols - Volatility and Jump Index markets
                const { active_symbols, error } = await apiInstance.send({ active_symbols: 'brief' });
    ...
    const authorizeIfNeeded = async () => {
        ...
        try {
            ...
            const loginid = authorize?.loginid || getLoginId();
